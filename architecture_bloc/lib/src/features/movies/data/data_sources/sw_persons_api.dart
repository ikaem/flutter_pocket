...
// this would probably be good to have a service for this, to be able to return data
  ...
  Future<List<MovieDTO>> many() async {
    await Future.delayed(const Duration(milliseconds: 500));

    final random = Random().nextInt(2);

    if (random == 0) throw const HttpException("Failed to fetch movies");

    return _moviesRaw;
  }

  Future<MovieDTO> one(int id) async {
    await Future.delayed(const Duration(milliseconds: 500));

    final random = Random().nextInt(2);

    if (random == 0) throw const HttpException("Failed to fetch the movei");

    final dto = _moviesRaw.firstWhereOrNull((person) {
      return person.id == id;
    });
